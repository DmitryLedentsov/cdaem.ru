    ...
    <div class="form-group required fort">
        <?php if ($type == 'confirm'): ?>
            ...
            <?php if ($userType == 'landlord') : ?>
                <p>
                    Сумма к оплате: <b><?=Yii::$app->formatter->asCurrency($priceToPay, Yii::$app->getModule('merchant')->viewMainCurrency)?></b>
                    <br>
                    Получаете при заселении Клиента: <b><?=Yii::$app->formatter->asCurrency($advertPrice - $priceToPay, Yii::$app->getModule('merchant')->viewMainCurrency)?></b>
                </p>
                <p>
                    После подтверждения заявки с Вашего счета будут автоматически списаны средства в размере <b><?=$this->context->module->ownerReservationPercent?>%</b> от суммы стоимости бронирования Вашего объекта.
                    Согласно правилам, Клиент бронируя ваше жильё вносит  5% от суммы брони, что является гарантией бронирования!
                    При заезде в ваше жильё Клиент, вносит оплату на 5% меньше, от стоимости бронирования вашего объекта!
                </p>
                <p style="color:red">
            ...
            <?php if ($userType == 'renter') : ?>
                ...
                <p>
                    Сумма к оплате: <b><?=Yii::$app->formatter->asCurrency($priceToPay, Yii::$app->getModule('merchant')->viewMainCurrency)?></b>
                    <br>
                    Платите Владельцу при заселении: <b><?=Yii::$app->formatter->asCurrency($advertPrice - $priceToPay, Yii::$app->getModule('merchant')->viewMainCurrency)?></b>
                </p>
                <p>
                    После подтверждения заявки с Вашего счета будут автоматически списаны средства в размере <b><?=$this->context->module->clientReservationPercent?>%</b> от суммы стоимости бронирования Вашего объекта,
            ...
            <p>Если на Вашем счету не окажется нужной суммы, Вы не сможете подтвердить заявку. Для этого пополните свой счет.</p>
            <p>Подробная информация доступна в разделе <strong><?=Html::a('пользовательское соглашение', ['/pages/default/index', 'url' => 'agreement'], ['target' => '_blanck'])?></strong>.</p>
...
echo Html::radioList($name, $selection, $items, [
    ...
    <div class="form-group text-center">
       <?php if ($user_id == 0) :  ?> 
        <input type="submit" value="<?=$type == 'confirm' ? 'Подтверждаю' : 'Отказ'?>" class="btn btn-primary" id="button-reservation-confirm" data-type="<?=$type?>" data-reservation="<?=$reservationId?>" data-user-id="<?=$user_id?>" data-priced="<?="".$priceToPay." Рублей " ?>" data-department="<?=$department?>"  data-user-type="<?=$userType?>"/>
        <?php else: ?>
          <div class="alert alert-warning">
                <b>Внимание</b> <br/>
