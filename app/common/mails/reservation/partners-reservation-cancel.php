...
<table class="row" width="100%" bgcolor="#ffffff" align="center" cellpadding="0" cellspacing="0" border="0"
    ...
    <tr>
        <td class="title"
            style="padding-top:5px; padding-right:30px; padding-bottom:20px; padding-left:30px; font-family:'Segoe UI', 'Helvetica Neue', Helvetica, Arial, sans-serif; font-size:27px; line-height:36px; color: red; font-weight:300;">
            <?= $sub ?>
        </td>
    </tr>
    <tr>
        <td style="padding-left:30px; padding-right:30px; padding-bottom:32px; font-family:Arial, Helvetica, sans-serif; font-size:12px; line-height:15pt; color:#777777;">
            ...
            <?php if ($userType == 'owner'): ?>
                <p>
                    Владелец апартаментов на сайте <?= $domain ?> отменил <br/> заявку на бронь
                    <b>№<?= $reservation->id ?></b>.
                </p>
            ...
            <?php else: ?>
                <p>
                    Клиент на сайте <?= $domain ?> отменил <br/> заявку на бронь под номером
                    <b>№<?= $reservation->id ?></b>.
                </p>
                <h2>Причина отмены:</h2>
                <?= Html::encode($reservation->cancel_reason) ?>
